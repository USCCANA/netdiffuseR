...
## `as_diffusionnet` function

The advantage of using this function is that, besides of been able to use the different methods for data visualization such as `plot.diffusionnet`, `heatgraph.diffusionnet`, `plot_threshold.diffusionnet`, etc., provides data check before use, in particular:
...
For a given graph with $n$ individuals in a network with $T$ time periods, the `diffusionnet-class` consists on the following list of objects:

Object  | Description
------- | -------
 `graph` | A dynamic graph defined as an array of size $n\times n\times T$, with one network for each slide 
