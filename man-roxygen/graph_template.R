...
#' <%=ifelse(exists("self") && self, "@param self Logical scalar. When \\code{TRUE} autolinks (loops, self edges) are allowed (see details).", "") %>
...
#' <%=ifelse(exists("undirected") && undirected, "@param undirected Logical scalar. When \\code{TRUE} only the lower triangle of the adjacency matrix will considered (faster).", "") %>
