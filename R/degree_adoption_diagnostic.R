...
degree_adoption_diagnostic <- function(
  ...
  if (Q > 1 && !is.null(behavior)) {
    if (is.character(behavior)) {
      ...
      if (any(behavior_indices < 1 | behavior_indices > Q)) {
        stop("behavior indices must be between 1 and ", Q)
      }
    } else {
  ...
  if (nrow(combined_data) < min_adopters) {
    warning("Insufficient adopters (n=", nrow(combined_data), ") for correlation analysis")
    return(create_empty_result(degree_strategy, original_call, combine, nrow(combined_data)))
  }
  ...
  undirected <- if (inherits(graph, "diffnet")) {
    ...
    # For other graph types, check if symmetric
...
process_graph_input <- function(graph, toa, t0, t1, name, ...) {
  ...
  graph <- new_diffnet(graph, toa, t0 = t0, t1 = t1, name = name, ...)
...
analyze_multi_behaviors_separately <- function(degrees, toa, min_adopters, bootstrap, R, conf.level, behavior_names, degree_strategy, original_call) {
  Q <- ncol(toa)
  ...
  for (q in seq_len(Q)) {
    ...
    if (length(adopters_q) >= min_adopters) {
      ...
      correlations_matrix[1, q] <- cor(data_q$indegree, data_q$toa, use = "complete.obs")
      correlations_matrix[2, q] <- cor(data_q$outdegree, data_q$toa, use = "complete.obs")
...
prepare_combined_data <- function(degrees, toa, combine, min_adopters, Q) {
  if (Q == 1 || combine == "pooled") {
    ...
    toa_min <- apply(toa, 1, min, na.rm = TRUE)
...
compute_correlations <- function(data) {
  c(
    indegree_toa = cor(data$indegree, data$toa, use = "complete.obs"),
    outdegree_toa = cor(data$outdegree, data$toa, use = "complete.obs")
  )
}
...
compute_bootstrap_results <- function(data, R, conf.level) {
  safe_bootstrap <- function(data, indices) {
    ...
    tryCatch({
      c(
        indegree_toa = cor(d$indegree, d$toa, use = "complete.obs"),
        outdegree_toa = cor(d$outdegree, d$toa, use = "complete.obs")
      )
...
check_undirected_graph <- function(graph) {
  if (is.array(graph) && length(dim(graph)) >= 2) {
    # Check if symmetric for first time slice
    if (length(dim(graph)) == 3) {
      return(isSymmetric(graph[,,1]))
    } else {
      return(isSymmetric(graph))
    }
  }
  FALSE  # Default to directed for unknown types
}
