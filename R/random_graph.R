...
rewire_graph <- function(graph, p, both.ends=FALSE, self=FALSE, multiple=FALSE,
  ...
  out <- switch(class(graph),
    ...
    list = rewire_graph.list(graph, p, both.ends, self, multiple, undirected, copy.first),
    ...
    array = rewire_graph.array(graph, p, both.ends, self, multiple, undirected, copy.first),
    stopifnot_graph(graph)
  )
...
# @rdname rewire_graph
rewire_graph.list <- function(graph, p, both.ends, self, multiple, undirected,
                              copy.first) {
  t   <- length(graph)
  out <- vector("list", t)
  ...
  for (i in 1:t) {
    ...
    out[[i]] <- rewire_graph_cpp(graph[[j]], p[i], both.ends, self, multiple,
                                    undirected)
    # Names
    rn <- rownames(graph[[i]])
...
# @rdname rewire_graph
rewire_graph.array <-function(graph, p, both.ends, self, multiple, undirected,
                              copy.first) {
  n   <- dim(graph)[1]
  t   <- dim(graph)[3]
  out <- vector("list", t)
