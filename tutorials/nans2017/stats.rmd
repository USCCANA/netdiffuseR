...
*   In this type of models we usually have the following
    ...
    y_{it} = \left\{
    \begin{array}{ll}
    1 & \mbox{if}\quad \gamma y_{it-1} + \rho\sum_{j\neq i}\frac{W_{ijt-1}y_{it-1}}{\sum_{j\neq i}W_{ijt-1}} > 0\\
    0 & \mbox{otherwise}
    \end{array}
    \right.
    $$
...
# Baseline network
W <- methods::getMethod("t", signature = "dgCMatrix")(rgraph_ws(n, k=10, p = .2))
...
ans <- glm(adopted ~ Measure + cohesive_exposure, family = binomial(link="probit"),
